...
public class AcknowledgementComp implements Callable, CoreCommonConstants
{
    ...
    private static final Logger log = LoggerFactory
            .getLogger(AcknowledgementComp.class);
    private static final String ID = "[InboundAcknowledgementComp]";
    ...
    public Object onCall(MuleEventContext ctx) throws Exception
    {
        ...
        if (batch.getSupplier().getDeploymentMode().equals(DeploymentMode.LOCAL))
        {
        ...
        boolean succ = true;
        boolean flag = false;// used to indicate whether any files copied into
                             // channel mailbox.
        ...
        for (DocMsg docMsg : batch.getDocs())
        {
            ...
            try
            {
                File archPath = this.getArchPath(docMsg.getBuyer(), yyyymm);
                File targetPath = this.getTargetPath(batch.getSupplier()
                        .getChannel());
    ...
    {
        return new File(mboxUtil.getChannelInPath(channelConfig
            .getChannelMailbox(channel)));//appConfig.getChannelMailboxRootPath() + PS + channelConfig.getChannelMailbox(channel) + PS + "in"
    }


    private File getArchPath(BuyerHolder buyer, String yyyymm)
            throws IOException
    {
        //File rlt = new File(appConfig.getBuyerMailboxRootPath() + PS
        //+ buyer.getMboxId() + PS + "archive" + PS + "in" + PS
        //+ yyyymm);
        File rlt = new File(mboxUtil.getFolderInBuyerArchInPath(buyer.getMboxId(), yyyymm));
