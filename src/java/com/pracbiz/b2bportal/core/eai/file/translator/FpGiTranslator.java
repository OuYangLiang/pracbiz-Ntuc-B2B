 ...
 * @author YinChi
 */
public class FpGiTranslator extends SourceTranslator
{
        ...
        throws Exception
    {
        ...
        Map<String, List<String>> giMap = null;
        ...
        try
        {
            giMap = new HashMap<String, List<String>>();
            for (Map.Entry<String, byte[]> entry : files.entrySet())
            {
                ...
                for (String content : contents)
                {
                    ...
                    String giNo = content.substring(2, 12).trim();
                    String supplierCode = content.substring(58, 68).trim();
                    String key = giNo +"@@"+ supplierCode + entry.getKey();
                    if (giMap.containsKey(key))
                    {
                        giMap.get(key).add(content + END_LINE);
                    }
                    else
                    {
            ...
            for (Map.Entry<String, List<String>> entry : giMap.entrySet())
            {
                fileName = outPath
                    + File.separator
                    + "GI_"
                ...
                if (fileNames.contains(fileName))
                {
                    ...
                    GZIPHelper.getInstance().doStandardZip(fileList, "GI", buyer.getBuyerCode(), outPath);
                }
                else
                {
                  //****** write target file *********
