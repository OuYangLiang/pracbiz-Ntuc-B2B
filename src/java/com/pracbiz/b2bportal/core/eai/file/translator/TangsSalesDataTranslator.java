...
import com.pracbiz.b2bportal.core.eai.file.canonical.SalesDocFileHandler;
 ...
 * @author YinChi
 */
public class TangsSalesDataTranslator extends SourceTranslator
{
    @Autowired SalesDocFileHandler canonicalSalesDocFileHandler;
        ...
        throws Exception
    {
        ...
        try
        {
            ...
            for (Map.Entry<String, List<String[]>> entry : dsdMap.entrySet())
            {
                ...
                if (fileNames.contains(fileName))
                {
                    ...
                    byte[] data = canonicalSalesDocFileHandler.getFileByte(salesHolder, file, DocFileHandler.CANONICAL);
                    
                    FileUtil.getInstance().writeByteToDisk(data, file.getPath());
                    
                    fileList.add(file);
                    fileList.add(newSrcFile);
                    GZIPHelper.getInstance().doStandardZip(fileList, "DSD", buyer.getBuyerCode(), outPath);
                }
                else
                {
                    //****** write target file *********
