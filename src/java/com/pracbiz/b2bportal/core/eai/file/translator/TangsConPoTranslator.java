 ...
 * @author YinChi
 */
public class TangsConPoTranslator extends SourceTranslator
{
    private static final Logger log = LoggerFactory.getLogger(TangsConPoTranslator.class);
        ...
        throws Exception
    {
        ...
        {
            contents = FileParserUtil.getInstance().readLines(entry.getValue());
        }
        ...
        if (contents != null)
        {
            for (int i = 1; i < contents.size(); i++)
            {
                ...
                {
                    concessMap.get(supplierCode).add(contents.get(i));
                }
                else
                {
                    List<String> groupContents = new ArrayList<String>();
                    groupContents.add(contents.get(i));
        ...
        String file = null;
        Map<String, byte[]> targetFiles = new HashMap<String, byte[]>();
        if (map != null && !map.isEmpty())
        {
            for (Map.Entry<String, PoHolder> entry : map.entrySet())
            {
                ...
                byte[] bytes = canonicalPoDocFileHandler.getFileByte(entry.getValue(), targetFile, PoDocFileHandler.CANONICAL);
                targetFiles.put(file, bytes);
            }
        }
