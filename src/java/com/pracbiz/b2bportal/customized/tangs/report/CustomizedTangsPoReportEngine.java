 ...
 * @author yinchi
 */
public class CustomizedTangsPoReportEngine extends DefaultPoReportEngine implements CoreCommonConstants
{
    private static final Logger log = LoggerFactory.getLogger(CustomizedTangsPoReportEngine.class);
        ...
        ReportEngineParameter<PoHolder> parameter, int flag)
    {
        ...
        try
        {
            ...
            if(null != parameter.getBuyer().getPostalCode())
            {
                map.put("BUYER_POSTAL_CODE", parameter.getBuyer().getPostalCode());
            }
            
            if(null != parameter.getBuyer().getContactEmail())
            {
                map.put("BUYER_CONTACT_EMAIL", parameter.getBuyer().getContactEmail());
            }
    ...
    protected String reportTemplate(PoHolder data, int flag)
    {
        if(PoType.SOR.equals(data.getPoHeader().getPoType()))
        {
            if (flag == 1 || flag == 3)
            {
                return "reports/customized/po/tangs/SOR" + CoreCommonConstants.REPORT_TEMPLATE_FOR_STORE_EXTENSION + ".jasper";
            }
            
            return "reports/customized/po/tangs/SOR.jasper";
        }
        else
        {
            if (flag == 1 || flag == 3)
            {
                return "reports/customized/po/tangs/CUSTOMIZED_TANGS_CON_PO" + CoreCommonConstants.REPORT_TEMPLATE_FOR_STORE_EXTENSION + ".jasper";
            }
            
            return "reports/customized/po/tangs/CUSTOMIZED_TANGS_CON_PO.jasper";
        }
    }
