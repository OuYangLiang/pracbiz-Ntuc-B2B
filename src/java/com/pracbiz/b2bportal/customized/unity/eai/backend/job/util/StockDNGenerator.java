 ...
 * @author youwenwu
 */
public class StockDNGenerator extends DnGenerator
{
    public StockDNGenerator(BusinessRuleService businessRuleService)
    {
            ...
            throws Exception
    {
        ...
        dnHeader.setDnType(DnType.STK_QOC.name());
            ...
            GrnHolder grnHolder) throws Exception
    {
        ...
        for (InvDetailHolder invDetail : invHolder.getDetails())
        {
            ...
            boolean flag = true;
            for (GrnDetailHolder grnDetail : grnHolder.getDetails())
            {
                if (grnDetail.getBuyerItemCode().equals(buyerItemCode))
                {
                    if (invDetail.getInvQty().compareTo(
                            grnDetail.getReceiveQty()) > 0)
                    {
                        BigDecimal debitQty = invDetail.getInvQty().subtract(grnDetail.getReceiveQty());
                        dnDetails.add(this.initDnDetail(dnOid,
                                invHolder.getHeader(), invDetail, index++, debitQty));
                        totalCost = totalCost.add(debitQty.multiply(invDetail
                                .getUnitPrice()));
                    }
                    flag = false;
                    break;
            ...
            if (flag)
            {
                dnDetails.add(this.initDnDetail(dnOid, invHolder.getHeader(),
                        invDetail, index++, invDetail.getInvQty()));
                totalCost = totalCost.add(invDetail.getItemAmount());
            }
        }
            ...
            InvDetailHolder invDetail, Integer lineSeqNo, BigDecimal debitQty)
    {
        ...
        dnDetail.setUnitCost(invDetail.getUnitPrice());
    ...
    {
        return businessRuleService.isAutoGenStockDn(buyerOid);
    }
