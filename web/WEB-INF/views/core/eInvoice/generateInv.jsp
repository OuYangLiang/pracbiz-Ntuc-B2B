                     ...
                     )
                 {
					...
					changeInvDetails = function(object,lineSeqNo)
                    {
                        ...
                        jsonStore.fetchItemByIdentity({identity:lineSeqNo,onItem:function(item)
                        {
                        	...
                        	if (objName == "focQty")
                        	{
                        		var poQty = jsonStore.getValue(item,"focQty");
        			...
        			if (dom.byId("saveBtn"))
                    {
                        ...
                        {
                    		save('<s:url value="/inv/saveGenerateInv.action"/>');
                        });
                    }
                    	...
                    	on(registry.byId("saveAndSentBtn"), 'click', function()
                        {
                    		var confirmDialog = new ConfirmDialog({
                                ...
                                yesBtnPressed: function(){
                                	save('<s:url value="/inv/saveAndSentGenerateInv.action"/>');
                                }
                            });
                        	confirmDialog.show();
                        });
                    ...
                    if (dom.byId("restBtn"))
                    {
                    	...
                    	{
                    		changeToURL('<s:url value="/inv/initGenerateInv.action" />?param.invOid=<s:property value="#session.oldInvOidToGenerate"/>');
                		}); 
                    }
