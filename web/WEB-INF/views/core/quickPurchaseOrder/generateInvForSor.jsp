        		...
        		[
                     ...
                     )
                 {
        			...
        			if (dom.byId("saveBtn"))
                    {
                        ...
                        {
                            save('<s:url value="/quickPo/save.action"/>');
                        });
                    }
                    	...
                    	on(registry.byId("saveAndSentBtn"), 'click', function()
                        {
                    		var confirmDialog = new ConfirmDialog({
                                ...
                                yesBtnPressed: function(){
                                	save('<s:url value="/quickPo/saveAndSent.action"/>');
                                }
                            });
                        	confirmDialog.show();
                        });
                    ...
                    if (dom.byId("restBtn"))
                    {
                    	on(registry.byId("restBtn"), 'click', function()
                    	{
                        	...
                        	changeToURL('<s:url value="/quickPo/generateInvForSor.action" />?docOid=' + docOid + "&store=" + store + "&invNo="+invNo +"&storeCount=" + storeCount + "&poExpiredMsg="+poExpiredMsg);
                		}); 
                    }
                    ...
                    if (dom.byId("cancelBtn"))
                    {
                    	on(registry.byId("cancelBtn"), 'click', function()
                    	{
                        	...
                        	if (storeCount > 1)
                        	{
                        		changeToURL('<s:url value="/quickPo/initGenerateInv.action" />?docOid=' + docOid + "&store=" + store + "&poExpiredMsg="+poExpiredMsg);
                        		return;
                            }
