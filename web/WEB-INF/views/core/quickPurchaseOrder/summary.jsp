...
<head>
    ...
    <script>
        		...
        		[
                     ...
                     )
                 {
                    ...
                    customGrid.initGrid(
                    		...
                    		'<s:url value="/quickPo/data.action" />',
                            '<s:url value="/ajax/findSummaryFieldAndTooltips.action"/>',
                            [{field: '<s:text name="quick.po.summary.grid.poNo" />',method: formatHref},
                             {field: '<s:text name="quick.po.summary.searcharea.poStatus" />',method: formatStatus},
                             {field: '<s:text name="quick.po.summary.grid.invNo" />',method: formatHrefInv}]);
                    ...
                    if (dom.byId("searchBtn"))
                    {
                        on(registry.byId("searchBtn"), 'click', 
                            function()
                            {
                                ...
                                xhr.post({
                                    url: '<s:url value="/quickPo/search.action" />',
                    ...
                    if (dom.byId("generateInvBtn"))
                    {
                    	on(registry.byId("generateInvBtn"), 'click', function()
                        {
                    		...
                    		var selectedOids = customGrid.grid.selection.preserver._selectedById;
                    		var selectedCount = 0;
                    		var docOid;
                        	...
                        	var url = '<s:url value="/quickPo/initGenerateInv.action" />';
                        	var cfmUrl = '<s:url value="/quickPo/initGenerateInv.action" />?docOid='+docOid;
							xhr.post({
                            	url: url,
                                handleAs: "json",
                        ...
                        yesBtnPressed: function(){
	                        ...
	                        xhr.post({
                                ...
                                load: function(printDocMsg)
                                {
                                	...
                                	{
				                        submitFormInNewWindow('searchForm', '<s:url value="/quickPo/print.action" />?selectAll=true');
                                	}
                                	else
                                	{
	                                    ...
	                                    else
	                                    {
	                                    	xhr.post({
	                                             ...
	                                             load: function(printDocMsg)
	                                             {
	                                             	if (printDocMsg == 'success')
	                                             	{
				                                        window.open('<s:url value="/quickPo/print.action" />');
	                                             	}
	                                             	else
	                                             	{
	                                             		new InformationDialog({message: printDocMsg}).show();
                    ...
                    var selectAllData = new SelectAllConfirmDialog({
                        message: '<s:text name="B2BPC1665" />',
                        yesBtnPressed: function(){
                        	var validPoFrom = dijit.byId("param.poDateFrom").getValue();
	                        var validPoTo = dijit.byId("param.poDateTo").getValue();
                                      ...
                                      else
                                      {
                                    	  xhr.post({
                                              ...
                                              load: function(printDocMsg)
                                              {
                                              	if (printDocMsg == 'success')
                                              	{
			                                          changeToURL('<s:url value="/quickPo/exportExcel.action" />');
                                              	}
                                              	else
                                              	{
                                              		new InformationDialog({message: printDocMsg}).show();
						...
						function()
                        {
	                        ...
	                        xhr.post({
                                ...
                                load: function(printDocMsg)
                                {
                                	if (printDocMsg == 'success')
                                	{
				                        submitForm('searchForm','<s:url value="/quickPo/exportSummaryExcel.action" />');
                                	}
                                	else
                                	{
                                		new InformationDialog({message: printDocMsg}).show();
							...
							function()
                            {
	                         ...
	                         xhr.get({
	                                ...
	                                load: function(data)
	                                {
	                                    ...
	                                    {
	                                    	xhr.get({
	        	                                url: '<s:url value="/quickPo/initEditDeliveryDate.action" />',
	        	                                handleAs: "json",
	        	                                load: function(data)
	        	                                {
                    ...
                    if (dom.byId("editInvoiceNoBtn"))
                    {
						on(registry.byId("editInvoiceNoBtn"),'click',
							function()
                            {
							 var selected = customGrid.getCountOfSelected();
							 if(selected < 1)
	                         ...
	                         xhr.get({
	                                ...
	                                load: function(data)
	                                {
	                                    ...
	                                    {
	                                    	xhr.get({
	        	                                url: '<s:url value="/quickPo/initEditInvoiceNo.action" />',
	        	                                handleAs: "json",
	        	                                load: function(data)
	        	                                {
		        	                                var errorMsg=data.errorMsg;
		        	                                var editPo=data.editPo;
		        	                                var editPoHex=data.editPoHex;
	        	                                	if (data.errorMsg != null && data.errorMsg != '')
	                                                {
	                                                    (new InformationDialog({message: data.errorMsg})).show();
	                                                }
	                                                else
	                                                {
	                                                	initEidtInvoiceNoDiv(editPo, editPoHex);
	                                                    registry.byId("editInvoiceNoDiv").show();
	                                                }
	        	                                }
	                                    	});
	                                    }
                   ...
                   var batchGenerateInv = function()
                   {
                	   var selected = customGrid.getCountOfSelected();
               		   ...
               		   if (customGrid.grid.selection.getSelectedCount() > 0 && customGrid.grid.selection.getSelectedCount() == customGrid.grid.rowCount)
                 	   {
                 		  selectType.show();
                 	   }
               		   else
                  	   {
               			  initBatchGenerateInv('<s:url value="/quickPo/checkInitBatchGenerateInv.action" />?selectType=false');
                  	   }
                   }
                    ...
                    var selectType = new SelectAllConfirmDialog({
                        ...
                        yesBtnPressed: function()
                        {
                        	...
                        	initBatchGenerateInv('<s:url value="/quickPo/checkInitBatchGenerateInv.action" />?selectType=true');
                        },
                        noBtnPressed: function()
                        {
                        	initBatchGenerateInv('<s:url value="/quickPo/checkInitBatchGenerateInv.action" />?selectType=false');
                    ...
                    var initBatchGenerateInv = function(checkUrl)
                    {
                    	var selectedOids = customGrid.grid.selection.preserver._selectedById;
                		var selectedCount = 0;
                		var oids = "";
                    	...
                    	var url = '<s:url value="/quickPo/initBatchGenerateInv.action" />';
                   	    xhr.get({
                              url: '<s:url value="/quickPo/putParamIntoSession.action" />',
                              content: {selectedOids: oids},
                              load: function(data)
                              {
                                  ...
                                  {
                                	  xhr.get({
        	                              url: checkUrl, //'<s:url value="/quickPo/checkInitBatchGenerateInv.action" />'
        	                              handleAs: "json",
        	                              load: function(data)
        	                              {
	        	                              if (data.errorMsg != null && data.errorMsg != '')
                    ...
                    refreshGridWithoutSelected = function()
                    {
                    	refreshGrid();
                    	customGrid.grid.selection.clear();
                    }
       ...
       {
    	  var bool = '<s:property value="#session.permitUrl.contains(\'/quickPo/print.action\')" />';
       ...
       var formatHrefInv = function(field,index,cell)
       {
          ...
          if(printInv == 'true' && printPo == 'true')
          {
         	  ...
         	  if (duplicate)
           	  {
         		  return "<a style=\"color: red\" href=\"javascript:doPrint("+docOid+", 'INV','" + field + "');\">"+ (field == null?'':field) +"</a>"
           	  }
         	  else
           	  {
         		  return "<a  href=\"javascript:doPrint("+docOid+", 'INV','" + field + "');\">"+ (field == null?'':field) +"</a>"
           	  }
           }
           else
           {
       ...
       var formatByStore = function(field,index,cell)
       {
        ...
        function doPrint(oid, docType, invNo)
        {
        	...
        	xhr.get({
                ...
                load: function(data)
                {
                    ...
                    {
                    	xhr.post({
			                url: '<s:url value="/quickPo/checkOtherPdfData.action" />?docType=' + docType + "&invNo=" + invNo,
			                content: {selectedOids: oid},
			                sync : true, 
			                load: function(data)
			                {
			                    ...
			                    {
			                    	window.open('<s:url value="/quickPo/print.action" />?docType=' + docType + "&invNo=" + invNo);
			                    }
			                }
			            });
                    }
        ...
        function saveEdit()
        {
        	...
        	xhr.post({
                url: '<s:url value="/quickPo/saveEditDeliveryDate.action" />',
        ...
        function cancelledEdit(divType)
        {
            if (divType == 'DEL')
            {
        ...
        function initEidtInvoiceNoDiv(editPo, editPoHex)
        {
			document.getElementById("editInvBuyerCode").innerHTML=editPo.buyerCode;
			document.getElementById("editInvBuyerName").innerHTML=editPo.buyerName;
			document.getElementById("editInvSupplierCode").innerHTML=editPo.supplierCode;
			document.getElementById("editInvSupplierName").innerHTML=editPo.supplierName;
			document.getElementById("editInvPoNo").innerHTML=editPo.poNo;
			document.getElementById("editInvPoDate").innerHTML=editPo.poDate.substr(0,10);
			document.getElementById("editInvPoType").innerHTML=editPo.poType;
			document.getElementById("editInvTotalCost").innerHTML=editPo.totalCost;
			document.getElementById("editInvoiceNo").value=(editPoHex == null ? "" : editPoHex.stringValue);
			document.getElementById("editPoOid").value=editPo.poOid;
        }


        function saveEditInvoiceNo()
        {
        	var B2BPortalBase = require("custom/B2BPortalBase");
        	(new B2BPortalBase()).resetTimeout(
                    '<s:property value="#session.commonParam.timeout" />',
        	...
        	var poOid = document.getElementById("editPoOid").value;
        	var invoiceNo = document.getElementById("editInvoiceNo").value;
        	var csrfToken = document.getElementById("csrfToken").value;
        	xhr.post({
                url: '<s:url value="/quickPo/saveEditInvoiceNo.action" />',
                content: {"editPoHex.poOid": poOid, "editPoHex.stringValue":invoiceNo, "csrfToken": csrfToken},
                handleAs: "json",
                load: function(data)
                {
                	var errorMsg=data.errorMsg;
                	...
                	else if (errorMsg=="invExist")
                	{
                		(new InformationDialog({message: '<s:text name="B2BPC1709" />'})).show();
                	}
                	else if (errorMsg=="invNoReg")
                	{
                		(new InformationDialog({message: '<s:text name="B2BPC1616" />'})).show();
                	}
                	else if (errorMsg=="invNoExceed")
                   	{
                		(new InformationDialog({message: "Tax Invoice No must not exceed " + data.maxInvNoLength + " characters."})).show();
                   	}
                	else if (errorMsg=="isEditting")
                	{
						return;
                    ...
                    {
                    	registry.byId("editInvoiceNoDiv").hide();
                    	refreshGridWithoutSelected();
                    	(new InformationDialog({message: '<s:text name="B2BPC1708" />'})).show();
                    }
                }
            });
        }
    ...
    <div dojoType="dijit.Dialog" id="editDeliveryDateDiv" title="Edit Delivery Date" style="width:50%;font-weight: bolder;">
    <form id="editForm" name="editForm" method="post">
    <input type="hidden" id="csrfToken" name="csrfToken" value="<s:property value='#session.csrfToken' />" />
        <table class="commtable">
            <tr class="odd">
                <td style="font-weight: bolder;"><s:text name="quick.po.edit.buyerCode" /></td>
                <td><span id="buyerCode" ></span></td>
                <td style="font-weight: bolder;"><s:text name="quick.po.edit.buyerName" /></td>
                <td><span id="buyerName" ></span></td>
            </tr>
            <tr class="even">
                <td style="font-weight: bolder;"><s:text name="quick.po.edit.supplierCode" /></td>
                <td><span id="supplierCode" ></span></td>
                <td style="font-weight: bolder;"><s:text name="quick.po.edit.supplierName" /></td>
                <td><span id="supplierName" ></span></td>
            </tr>
            <tr class="odd">
                <td style="font-weight: bolder;"><s:text name="quick.po.edit.poNo" /></td>
                <td><span id="poNo" ></span></td>
                <td style="font-weight: bolder;"><s:text name="quick.po.edit.poDate" /></td>
                <td><span id="poDate" ></span></td>
            </tr>
            <tr class="even">
                <td style="font-weight: bolder;"><s:text name="quick.po.edit.poType" /></td>
                <td><span id="poType" ></span></td>
                <td style="font-weight: bolder;"><s:text name="quick.po.edit.totalCost" /></td>
                <td><span id="totalCost" ></span></td>
            </tr>
            <tr class="odd">
                <td style="font-weight: bolder;"><s:text name="quick.po.edit.deliveryDate" /></td>
                <td colspan="3">
            ...
            <tr>
                <td colspan="4" style="text-align:left">
                   ...
                   <button data-dojo-type="dijit.form.Button" onclick="cancelledEdit('DEL');"><s:text name="button.cancel"/></button>
               </td>
           </tr>
        </table>
    </form>
    ...
    <div dojoType="dijit.Dialog" id="editInvoiceNoDiv" title="Edit Invoice No" style="width:50%;font-weight: bolder;" >
    <form id="editInvoiceNoForm" name="editInvoiceNoForm" method="post">
    <input type="hidden" id="csrfToken" name="csrfToken" value="<s:property value='#session.csrfToken' />" />
        <table class="commtable">
            <tr class="odd">
                <td  style="font-weight: bolder;">&nbsp;<s:text name="quick.po.edit.buyerCode" /></td>
                <td><span id="editInvBuyerCode" ></span></td>
                <td  style="font-weight: bolder;">&nbsp;<s:text name="quick.po.edit.buyerName" /></td>
                <td><span id="editInvBuyerName" ></span></td>
            </tr>
            <tr class="even">
                <td  style="font-weight: bolder;">&nbsp;<s:text name="quick.po.edit.supplierCode" /></td>
                <td><span id="editInvSupplierCode" ></span></td>
                <td  style="font-weight: bolder;">&nbsp;<s:text name="quick.po.edit.supplierName" /></td>
                <td><span id="editInvSupplierName" ></span></td>
            </tr>
            <tr class="odd">
                <td  style="font-weight: bolder;">&nbsp;<s:text name="quick.po.edit.poNo" /></td>
                <td><span id="editInvPoNo" ></span></td>
                <td  style="font-weight: bolder;">&nbsp;<s:text name="quick.po.edit.poDate" /></td>
                <td><span id="editInvPoDate" ></span></td>
            </tr>
            <tr class="even">
                <td  style="font-weight: bolder;">&nbsp;<s:text name="quick.po.edit.poType" /></td>
                <td><span id="editInvPoType" ></span></td>
                <td  style="font-weight: bolder;">&nbsp;<s:text name="quick.po.edit.totalCost" /></td>
                <td><span id="editInvTotalCost" ></span></td>
            </tr>
            <tr class="odd">
                <td  style="font-weight: bolder;">&nbsp;<s:text name="quick.po.edit.invoice.no" /></td>
                <td colspan="3">
                	&nbsp;<input type="text" id="editInvoiceNo" name="editPo.invoiceNo" 
		            	data-dojo-type="dijit/form/TextBox" maxlength="20"
		            	value='<s:text name="param.invoiceNo" />'/>
		            <input id="editPoOid" name="editPo.poOid" style="display : none" />
		        </td>
            </tr>
            <tr class="space"><td></td></tr>
            <tr>
                <td colspan="4" style="text-align:left">
                   <button data-dojo-type="dijit.form.Button"  onclick="saveEditInvoiceNo();" ><s:text name="button.ok"/></button>
                   <button data-dojo-type="dijit.form.Button" onclick="cancelledEdit('INV');"><s:text name="button.cancel"/></button>
               </td>
           </tr>
        </table>
    </form>
...
<body>
    ...
    <div>
        <table class="btnContainer"><tbody><tr><td>
        	<s:if test="#session.permitUrl.contains('/quickPo/search.action')" >
            	<button data-dojo-type="dijit/form/Button" id="searchBtn" ><s:text name="button.search" /></button>
            </s:if>
            <s:if test="#session.permitUrl.contains('/quickPo/print.action')" >
            	<button data-dojo-type="dijit/form/Button" id="printBtn" ><s:text name="button.print" /></button>
            </s:if>
            <s:if test="#session.permitUrl.contains('/quickPo/exportExcel.action')" >
            	<button data-dojo-type="dijit/form/Button" id="exportExcel" ><s:text name="button.exportExcel" /></button>
            </s:if>
            <s:if test="#session.permitUrl.contains('/quickPo/exportSummaryExcel.action')" >
            	<button data-dojo-type="dijit/form/Button" id="exportSummaryExcel" ><s:text name="button.exportSummaryExcel" /></button>
            </s:if>
             <s:if test="#session.permitUrl.contains('/quickPo/initEditInvoiceNo.action')" >
            	<button data-dojo-type="dijit/form/Button" id="editInvoiceNoBtn" ><s:text name="button.editInvoiceNo" /></button>
            </s:if>
            <s:if test="#session.permitUrl.contains('/quickPo/generateInv.action')" >
    ...
    <div data-dojo-type="dijit/TitlePane" data-dojo-props="title:'<s:text name="quick.po.summary.searcharea.headerLabel" />', width:275">
        <form id="searchForm" name="searchForm" method="post" >
        <table class="commtable">
            <tbody>
				<tr>
                	<td width="85"><s:text name="quick.po.summary.searcharea.buyer" /></td>
                	<td>:</td>
                	<td>
                	...
                	<td width="85"><s:text name="quick.po.summary.searcharea.storeCode" /></td>
                    <td>:</td>
                    <td>
				...
				<tr>
                	<td><s:text name="quick.po.summary.searcharea.supplierName" /></td>
                	<td>:</td>
                	<td>
                	...
                	<td><s:text name="quick.po.summary.searcharea.supplierCode" /></td>
                	<td>:</td>
                	<td>
             	...
             	<tr>
                	<td><s:text name="quick.po.summary.searcharea.supplier" /></td>
                	<td>:</td>
                	<td>
                	   <s:select data-dojo-type="dijit/form/FilteringSelect" 
             	...
             	<tr>
                	<td><s:text name="quick.po.summary.searcharea.poNo" /></td>
                	<td>:</td>
                	<td>
                	...
                	<td><s:text name="quick.po.summary.searcharea.deliveryDate" /></td>
                	<td>:</td>
                	<td>
                		<label for="Dfrom1"><s:text name="Value.from" /></label>
             	...
             	<tr>
                	<td><s:text name="quick.po.summary.searcharea.poDate" /></td>
                	<td>:</td>
                	<td>
                		<label for="Dfrom1"><s:text name="Value.from" /></label>
                	...
                	<s:if test="#session.SESSION_CURRENT_USER_PROFILE.buyerOid != null" >  
                	<td><s:text name="quick.po.summary.searcharea.sentDate" /></td>
                	<td>:</td>
                	<td>
                		<label for="Dfrom1"><s:text name="Value.from" /></label>
	             	...
	             	<s:if test="#session.SESSION_CURRENT_USER_PROFILE.supplierOid != null" >  
	                	<td><s:text name="quick.po.summary.searcharea.receivedDate" /></td>
	                	<td>:</td>
	                	<td>
	                		<label for="Dfrom1"><s:text name="Value.from" /></label>
             ...
             <tbody>
             	<tr>
                	<td width="85"><s:text name="quick.po.summary.searcharea.poStatus" /></td>
                	<td>:</td>
                	<td>
                	...
                	<td width="85"><s:text name="quick.po.summary.searcharea.readStatus" /></td>
                	<td>:</td>
                	<td>
		                <s:select data-dojo-type="dijit/form/Select" 
                	...
                	<td width="85"><s:text name="quick.po.summary.searcharea.favouriteList" /></td>
                    <td>:</td>
                    <td>
                        <s:select data-dojo-type="dijit/form/Select" 
