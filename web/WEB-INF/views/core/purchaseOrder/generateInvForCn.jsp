        		...
        		[
                     ...
                     )
                 {
					...
					changeInvDetails = function(object,lineSeqNo)
                    {
                        ...
                        jsonStore.fetchItemByIdentity({identity:lineSeqNo,onItem:function(item)
                        {
							...
							if (objName == "netAmount")
							{
								...
								jsonStore.setValue(item, "itemGrossAmount", itemGrossCost);
								jsonStore.setValue(item, "netAmount", parseFloat(netAmount).toFixed(2));
							...
							if (objName == "itemSharedCost")
							{
								if (!validateNumber(objValue) || objValue < 0)
								{
									...
									dojo.byId("itemSharedCost" + lineSeqNo).value=parseFloat(value).toFixed(2);
									return;
								}
						    ...
						    var netAmount = jsonStore.getValue(item, "netAmount");
						    var itemSharedCost = jsonStore.getValue(item, "itemSharedCost");
						    var discountAmount = jsonStore.getValue(item, "discountAmount");
						    jsonStore.setValue(item, "conTotalCost", parseFloat(netAmount) + parseFloat(itemSharedCost));
        			...
        			if (dom.byId("saveBtn"))
                    {
                        ...
                        {
                    		if (parseFloat(header.totalPay) >= 0)
                    		{
                    		...
                    		else
                       		{
                    			var confirmDialog = new ConfirmDialog({
	                                message: '<s:text name="B2BPC1659" />',
	                                yesBtnPressed: function(){
	                                	save('<s:url value="/po/saveCn.action"/>');
	                                }
	                            });
	                        	confirmDialog.show();
                       		}
                    	...
                    	on(registry.byId("saveAndSentBtn"), 'click', function()
                        {
							...
							else
							{
								var confirmDialog = new ConfirmDialog({
	                                message: '<s:text name="B2BPC1659" />',
	                                yesBtnPressed: function(){
	                                	save('<s:url value="/po/saveAndSentCn.action"/>');
	                                }
	                            });
	                        	confirmDialog.show();
							}
                    ...
                    if (dom.byId("restBtn"))
                    {
                    	on(registry.byId("restBtn"), 'click', function()
                    	{
                        	...
                        	changeToURL('<s:url value="/po/generateInvForCn.action" />?docOid=' + docOid);
                		}); 
                    }
					...
					<tbody>
						...
						<tr class="odd">
							...
							<td class="tdData">
								...
								<s:if test='invoice.header.shipToName != "" and invoice.header.shipToName != null'>
